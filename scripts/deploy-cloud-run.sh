...
CHAT_PROVIDER="${CHAT_LLM_PROVIDER:-google}"
...
[[ "$CHAT_PROVIDER" == "google" || "$CHAT_PROVIDER" == "vertex" || "$CHAT_PROVIDER" == "anthropic" || "$CHAT_PROVIDER" == "openai" ]] || {
  echo "CHAT_LLM_PROVIDER is invalid." >&2
  exit 2
}
[[ "$EMBEDDING_RUNTIME_PROVIDER" == "google" || "$EMBEDDING_RUNTIME_PROVIDER" == "vertex" ]] || {
  echo "EMBEDDING_PROVIDER is invalid." >&2
  exit 2
}
...
"$GCLOUD_BIN" run deploy "$SERVICE" \
  ...
  --update-env-vars="CHAT_LLM_PROVIDER=$CHAT_PROVIDER,EMBEDDING_PROVIDER=$EMBEDDING_RUNTIME_PROVIDER,CHAT_GOVERNANCE_MODE=$GOVERNANCE_MODE,CHAT_OBSERVABILITY_ENABLED=$OBSERVABILITY_ENABLED,CHAT_TRUSTED_PROXY_HOPS=$TRUSTED_PROXY_HOPS,CHAT_IP_ACTIVE_KEY_VERSION=v1,CHAT_IP_RETENTION_DAYS=7,CHAT_CONVERSATION_RETENTION_DAYS=30,CHAT_AUDIT_RETENTION_DAYS=90" \
