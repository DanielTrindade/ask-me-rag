...
describe('Vertex ADC configuration', () => {
  it('resolve projeto/localização por papel e mantém isolamento', () => {
    const env = {
    ...
    expect(resolveVertexSettings('chat', env)).toEqual({
      project: 'chat-project',
      location: 'us-central1',
    });
    expect(resolveVertexSettings('embedding', env)).toEqual({
  ...
  it.each([
    ...
    expect(() => resolveVertexSettings('chat', {
      CHAT_VERTEX_PROJECT: 'project',
      CHAT_VERTEX_LOCATION: 'us-central1',
      [variable]: 'forbidden',
    })).toThrow(variable);
  });
  ...
  it('entrega somente projeto/localização ao provider oficial', () => {
    ...
    expect(createVertexRuntimeProvider('chat', {
      CHAT_VERTEX_PROJECT: 'project',
      CHAT_VERTEX_LOCATION: 'us-central1',
  ...
  it('detecta seleção Vertex independente', () => {
    expect(usesVertex({ CHAT_LLM_PROVIDER: 'vertex' })).toBe(true);
    expect(usesVertex({ EMBEDDING_PROVIDER: 'vertex' })).toBe(true);
    expect(usesVertex({ CHAT_LLM_PROVIDER: 'google', EMBEDDING_PROVIDER: 'google' })).toBe(false);
  });
});
